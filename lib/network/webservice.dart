import 'package:films_app/logger/logger.dart';
...
class MovieDBApi {
  ...
  static String similarMovies(int id) {
    return 'https://api.themoviedb.org/$id/similar/'
        '?api_key=$_apiKey'
        '&language=$localeCode';
  }
  ...
  static Future<List<Movie>> getSimilar(id) async {
    http.Response response =
        await http.get(Uri.encodeFull(similarMovies(id)), headers: {
      "Content-type": "application/json",
    });

    DLog.d('getSimilar : ' + response.statusCode.toString());
